  ...
  return (
    <main className="admin-content">
      ...
      <div className="admin-metrics">
        ...
        <article><span>Organizaciones</span><strong>{overview.metrics.organizations}</strong><small>{overview.metrics.active_keys} API keys activas</small></article>
      ...
      <section className="admin-card">
        ...
        <div className="admin-activity-head"><span>Request ID</span><span>Organización</span><span>Clave</span><span>Estado</span><span>Latencia</span><span>Fecha</span></div>
        {overview.activity.map((request) => (
          <div className="admin-activity-row" key={request.request_id}>
            <code>{request.request_id}</code><span>{request.organization}</span><span>{request.key_name ?? "Revocada"}</span><span className={request.status_code >= 400 ? "admin-status error" : "admin-status"}>{request.status_code}</span><span>{request.duration_ms} ms</span><time>{formatTime(request.created_at)}</time>
          </div>
        ))}
